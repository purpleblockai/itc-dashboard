...
export default function BrandEvaluationPage() {
  ...
  const columns: ColumnDef<{
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const mrp = typeof mrpRaw === "string" ? parseFloat(mrpRaw) : (mrpRaw as number)
        const sp  = typeof spRaw === "string" ? parseFloat(spRaw) : (spRaw as number)
  ...
  function getBrandProductAverages(productData: any[], productName: string, brand: string) {
    const items = productData.filter((p: any) => p.name === productName && p.brand === brand);
    if (!items.length) return null;
    const avg = (arr: number[]) => arr.length ? arr.reduce((a, b) => a + b, 0) / arr.length : null;
    const valid = (arr: number[]) => arr.filter((v) => typeof v === 'number' && !isNaN(v));
    ...
    const discounts = (mrps.length && sellingPrices.length)
      ? mrps.map((mrp, i) => (mrp && sellingPrices[i] ? ((mrp - sellingPrices[i]) / mrp) * 100 : null)).filter(v => v !== null)
      : [];
    return {
  ...
  const brandComparisonData = products.map((productName) => {
    ...
    brands.forEach((brand) => {
      ...
      row[`${brand}_discount`] = avg && avg.discount !== null ? avg.discount : null;
      row[`${brand}_mrp`] = avg && avg.mrp !== null ? avg.mrp : null;
      row[`${brand}_sellingPrice`] = avg && avg.sellingPrice !== null ? avg.sellingPrice : null;
      row[`${brand}_availability`] = avg && avg.availability !== null ? avg.availability : null;
    });
    return row;
  });
  ...
  if (brandData && brandData.length > 0) {
    const missingAvgDiscount = brandData.filter(b => typeof b.avgDiscount !== 'number' || isNaN(b.avgDiscount));
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card className="card-hover col-span-2 h-[500px]">
          <CardHeader>
            ...
            <CardDescription>
              Positioning map of brands by discount percentage and product availability
            </CardDescription>
          </CardHeader>
        ...
        <Card className="card-hover">
          <CardHeader>
            ...
            <CardDescription>Average discount percentage by brand</CardDescription>
          </CardHeader>
        ...
        <Card className="card-hover">
          <CardHeader>
            ...
            <CardDescription>Product availability percentage by brand</CardDescription>
          </CardHeader>
          <CardContent className="h-[300px]">
            {isLoading ? (
              <Skeleton className="h-full w-full" />
